...
describes what users get. Pick a format with `-o`, or let imomath choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
imomath <command> -o table   # aligned columns for reading
imomath <command> -o jsonl   # one JSON object per line, for piping
imomath <command> -o json    # a single JSON array
imomath <command> -o csv     # spreadsheet friendly
imomath <command> -o tsv     # tab-separated
imomath <command> -o url     # just the URL column
imomath <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
imomath <command> --fields id,title,url
```
...
```bash
imomath <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
imomath <command>            # a table, because this is a terminal
imomath <command> | wc -l    # JSONL, because this is a pipe
```
