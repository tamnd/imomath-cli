// Package cli assembles the imomath command tree from the imomath
...
// NewApp assembles the kit application from the imomath domain. The
...
//
// To add a command, declare it in imomath/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := imomath.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(imomath.Domain{}).Register(app)
